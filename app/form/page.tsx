...
const APPROVALS = [
  { label: 'Newsletter Draft', desc: 'Subject line, opening note, Aerialosophy tip, class spotlight, and CTA — reviewed before sending.' },
  { label: 'Aerialosophy Blog Draft', desc: 'Elena expands your tip into a full blog post. You approve the draft directly in Squarespace before it goes live.' },
...
export default function FormPage() {
  const [f, setF] = useState({
    ...
    classNotes: '', aerialophyNote: '', notes: '',
  })
  ...
  const submit = async () => {
    ...
    try {
      const res = await fetch('/api/submit', {
        ...
        body: JSON.stringify({ ...f, audience: audience.join(', '), classes: [f.week1Class, f.week2Class, f.week3Class, f.week4Class].filter(Boolean).join(' | '), approvals: approvals.join(', ') })
      })
      if (!res.ok) throw new Error()
      setSavedMonth(f.month)
      setStatus('success')
      setF({ month: '', date: new Date().toISOString().split('T')[0], news: '', promo: '', topic: '', tip: '', story: '', quote: '', studentOfMonth: '', teacherOfMonth: '', week1Class: '', week2Class: '', week3Class: '', week4Class: '', classNotes: '', aerialophyNote: '', notes: '' })
  ...
  return (
    <div>
      ...
      <div style={styles.body}>
        {status === 'success' && <div style={styles.successBox}><span style={{ fontSize: '18px' }}>✅</span><div>Saved! Thank you, Zina. Everything received for <strong>{savedMonth}</strong>.</div></div>}
        ...
        <div style={styles.card}>
          ...
          <div style={styles.cardBody}>
            <div style={styles.noteBox}><strong style={{ fontWeight: 700 }}>How this works:</strong> You share your tip here in your own words. Elena will expand it into a full blog post and publish it on the website. You&apos;ll receive a draft to approve directly in Squarespace before it goes live — no extra work needed from you!</div>
            ...
            <div><label style={styles.fieldLabel}>Your Tip — In Your Own Words *</label><div style={styles.fieldHint}>Bullet points, a paragraph, a brain dump — all good. We&apos;ll shape it into the final content.</div><textarea style={{ ...styles.textarea, minHeight: '140px' }} value={f.tip} onChange={upd('tip')} placeholder="e.g. Most students hold their breath when working hard..." /></div>
            <div><label style={styles.fieldLabel}>Who Is This Tip For?</label><div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginTop: '4px' }}>{AUDIENCE.map(a => <Pill key={a} label={a} selected={audience.includes(a)} onClick={() => toggle(audience, a, setAudience)} />)}</div></div>
            <div><label style={styles.fieldLabel}>Student Story or Example</label><div style={styles.fieldHint}>A student win, a class moment — makes the content come alive.</div><textarea style={styles.textarea} value={f.story} onChange={upd('story')} placeholder="e.g. Last week, a student tried the breathing technique and immediately felt the difference..." /></div>
            <div><label style={styles.fieldLabel}>One-Sentence Takeaway</label><div style={styles.fieldHint}>This becomes the Tuesday social media quote card.</div><input style={styles.input} value={f.quote} onChange={upd('quote')} placeholder="e.g. You don't need more strength — you need to stop fighting your own body." /></div>
            <div><label style={styles.fieldLabel}>Anything Else for the Blog?</label><div style={styles.fieldHint}>Specific points or research you want Elena to include when expanding this into the full blog post.</div><textarea style={{ ...styles.textarea, minHeight: '75px' }} value={f.aerialophyNote} onChange={upd('aerialophyNote')} placeholder="e.g. Mention Golgi Tendon Bodies. Include the breathing exercise from my Level 2 class." /></div>
          </div>
        </div>
        <div style={styles.card}>
          <div style={styles.cardHead}><div style={styles.cardNum}>3</div><div style={styles.cardTitle}>Class Spotlight This Month</div></div>
          <div style={styles.cardBody}>
            <div style={styles.noteBox}><strong style={{ fontWeight: 700 }}>One class promoted per week</strong> on Facebook and Instagram — a dedicated Thursday post with class benefits, schedule, and sign-up link. Pick the 4 classes to push this month.</div>
            <div>
              <label style={styles.fieldLabel}>Assign One Class Per Week</label>
              <div style={styles.fieldHint}>Each gets its own dedicated social media promo post.</div>
            ...
            <div><label style={styles.fieldLabel}>Anything Special to Highlight?</label><div style={styles.fieldHint}>New time slots, limited spots, seasonal angle, or anything to emphasize.</div><textarea style={styles.textarea} value={f.classNotes} onChange={upd('classNotes')} placeholder="e.g. Bungee GOLD is now Wednesdays at 11am. Aerial Silks Intro filling fast — mention limited spots." /></div>
          </div>
        </div>
        <div style={styles.card}>
          <div style={styles.cardHead}><div style={styles.cardNum}>4</div><div style={styles.cardTitle}>Student &amp; Teacher of the Month</div></div>
          <div style={styles.cardBody}>
            <div style={styles.infoBox}>Student responses are collected separately through the student form and saved in Google Sheets. Just provide the names here so we can feature them in the newsletter and social posts!</div>
            <div style={styles.twoCol}>
              <div><label style={styles.fieldLabel}>Student of the Month</label><div style={styles.fieldHint}>First name is fine if they prefer privacy.</div><input style={styles.input} value={f.studentOfMonth} onChange={upd('studentOfMonth')} placeholder="e.g. Sandra" /></div>
              <div><label style={styles.fieldLabel}>Teacher of the Month</label><div style={styles.fieldHint}>Which teacher shone brightest this month?</div><input style={styles.input} value={f.teacherOfMonth} onChange={upd('teacherOfMonth')} placeholder="e.g. Coach Maria" /></div>
            </div>
          </div>
        </div>
        <div style={styles.card}>
          ...
          <div style={styles.cardBody}>
            <p style={{ fontSize: '12.5px', color: MUT, lineHeight: 1.6 }}>Check each item once reviewed and approved. Anything unchecked will be flagged before it goes live.</p>
        ...
        <div style={styles.card}>
          ...
          <div style={styles.cardBody}>
            <div><label style={styles.fieldLabel}>Open Notes</label><div style={styles.fieldHint}>Ideas, upcoming events, feedback on last month, anything at all.</div><textarea style={{ ...styles.textarea, minHeight: '120px' }} value={f.notes} onChange={upd('notes')} placeholder="e.g. Thinking of an open house in June. Want to cover nervous system work next month?" /></div>
          </div>
        </div>
