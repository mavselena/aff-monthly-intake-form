...
export async function insertResponse(data: Record<string, string>) {
  ...
  const result = await sql`
    INSERT INTO responses (
      ...
      aerialsophy_note, week1_class, week2_class, week3_class, week4_class,
      ...
      ${data.aerialsophy_note}, ${data.week1Class}, ${data.week2Class},
...
export async function updateResponse(id: number, data: Record<string, string>) {
  const sql = getDb()
  await sql`
    UPDATE responses SET
      ...
      story = ${data.story},
      quote = ${data.quote},
      aerialsophy_note = ${data.aerialsophy_note},
      week1_class = ${data.week1_class},
      week2_class = ${data.week2_class},
      week3_class = ${data.week3_class},
      week4_class = ${data.week4_class},
      class_notes = ${data.class_notes},
      student_of_month = ${data.student_of_month},
      teacher_of_month = ${data.teacher_of_month},
